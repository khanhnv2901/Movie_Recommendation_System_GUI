...
# Function to add the role column if it doesn't exist
def add_role_column():
    try:
            ...
            if st.button("Sign Up"):
                ...
                if data:
                    ...
                    if len(new_user) == 0 and len(new_password) == 0:
                        ...
                        add_userdata(new_user, new_password, 'user')
...
def manage_users():
    ...
    action = st.selectbox("Action", ["Add User", "Update User", "Delete User"])
    ...
    if action == "Add User":
        ...
        if st.button("Update User"):
            update_user(username, password, role if role else None)
